...
GfxBoard::GfxBoard(U32 nbHorses, U16 screensizeX, U16 screensizeY) noexcept : Board(nbHorses), m_screensizeX(screensizeX), m_screensizeY(screensizeY) {
    ...
    for (const auto& gfxHorseFile : gfxHorseFiles) {
        LoadHorsesBitmap(gfxHorseFile.file, gfxHorseFile.size);
    }
...
GfxBoard::~GfxBoard() noexcept {
    for (const auto& horseSprite : m_horseSprites) {
        delete horseSprite;
    }
    ...
    for (const auto& playerladderText : m_playerladderText) {
        delete playerladderText;
    }
...
bool GfxBoard::LoadHorsesBitmap(const char *file, std::size_t size) noexcept {
...
bool GfxBoard::ComputeGfxCoordinate() noexcept {
    ...
    for (const auto& playerladderText : m_playerladderText) {
        AdaptRatioForText(playerladderText);
    }
    AdaptRatioForText(m_startCaseText);
    for (const auto& horseSprite : m_horseSprites) {
        AdaptRatioForSprite(horseSprite);
    }

    return true;
}

void GfxBoard::AdaptRatioForSprite(sf::Sprite *sprite) noexcept {
...
void GfxBoard::AdaptRatioForText(sf::Text *text) noexcept {
    assert(text != nullptr);
    if (text != nullptr) {
...
float GfxBoard::getComputedRatio(float currentSizeX, float currentSizeY, int wantedSizeX, int wantedSizeY) noexcept {
...
bool GfxBoard::displayBoard(tpCaseList &) noexcept {
    return displayBoard();
}
...
bool GfxBoard::displayBoard() noexcept {
    ...
    for (const auto& caseItem : getCasesList()) {
        ...
        if (caseItem.isAStartCase()) {
            ...
            for (const auto& player : this->getPlayersList()) {
                if (caseItem.isAStartCaseForPlayer(&player)) {
                    ...
                    pText->setColor(*colors[player.getPlayerNb()]);
                    break;
                }
            }
            ...
            for (const auto& player : this->getPlayersList()) {
                if (caseItem.isALadderForPlayer(&player)) {
                    pText = m_playerladderText[caseItem.getLadderCaseValue() - 1];
                    pText->setColor(*colors[player.getPlayerNb()]);
                    break;
                }
            }
        ...
        S32 x = 0, y = 0;
        U16 sizeX = 0, sizeY = 0;
        if (pText != nullptr || pSprite != nullptr) {
...
void GfxBoard::displayHistoric() noexcept {
    m_pApp->clear();
    int xPos = 0;
    int yMaxPos = 0;
    int iPlayer = 0;
    ...
    for (tHistoricList::iterator it = getHistoricList().begin();
            ...
            it++, xPos++) {//When item is erased, iterator points on next item! So if iterator is incremented, iterator skips one item!
...
void GfxBoard::drawLine(float x1, float y1, float x2, float y2, const sf::Color *color) noexcept {
