...
bool Case::getPossibleMoves(Horse* horse, tMapDieNumberCase &outputMoves, unsigned int level) const {
    ...
    if (horse->isSleeping() && isAStartCaseForHorse(horse)) {
        assert(level == 0);
        if (m_pHorse == nullptr) {//case is free (no horse on the case)
            //return m_pNextCase->nbPossibleMove() + 1;
            outputMoves[getLadderCaseValue()] = this->getCaseId();
            return true;
        } else //there is a horse on the case
        {
            if (horse->haveSamePlayerOwner(m_pHorse)) {//return 0;//don't have the right to eliminate the horse of the same player
                return true;
            } else {//return 1;//have the right to eliminate the horse of the opponent!
                outputMoves[getLadderCaseValue()] = this->getCaseId();
                return true;
            }
        }
        ...
        if (Case::isValidCaseId(m_pNextCase)) {
            ...
            if (pHorseOnNextCase == nullptr) {//case is free (no horse on the case)
                //return m_pNextCase->nbPossibleMove() + 1;
        ...
        } else {//should not occurs, because there is always a successor to a normal case
            return false;
        }
    } else //if it is a ladder case or a base normal case of ladder
    {
        ...
        } else //there is a horse on the case
        {
            if (pHorseOnNextCase->haveSamePlayerOwner(horse)) {
                //return 0;//don't have the right to eliminate the horse of the same player
                return true;
            } else {//in the standard game, it is not possible to have opponent horses on the ladder
                //return 1;//have the right to eliminate the horse of the opponent!
                if (outputMoves.size() == 0)//ladder move is always unic, no other moves are possible!
                {
                    outputMoves[m_pBoard->getCase(m_pNextLadderCase).getLadderCaseValue()] = m_pNextLadderCase;
...
bool Case::moveFrom(tCaseId pCaseSource, Horse* pHorse) {
    ...
    if ((Case::isValidCaseId(pCaseSource) && m_pBoard->getCase(pCaseSource).getHorse() != nullptr)
            || pHorse != nullptr) {
        Horse* pHorseToMove;
        if (pHorse != nullptr) {//Horse is currently not on the board (so not on a case), but in its rest box.
            pHorseToMove = pHorse;
        } else {//Horse is on the board
...
bool Case::isALadderCase() const {
    return isALadderCase(m_caseValue);
}

bool Case::isANormalCase() const {
    return isANormalCase(m_caseValue);
}

bool Case::isAPureNormalCase() const {
    return isAPureNormalCase(m_caseValue);
}

bool Case::isAStartCase() const {
    return isAStartCase(m_caseValue);
}

bool Case::isABaseForLadder() const {
    return isABaseForLadder(m_caseValue);
}

bool Case::isAFinishCase() const {
    return isAFinishCase(m_caseValue);
}

bool Case::isAPureLadderCase() const {
    return isAPureLadderCase(m_caseValue);
}
